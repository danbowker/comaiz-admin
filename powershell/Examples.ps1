...
Write-Host "`n=== Example 2: Register New User ===" -ForegroundColor Cyan
try {
    $newUserToken = Register-ComaizUser `
        -BaseUrl $baseUrl `
        -Username "powershelluser" `
        -Email "powershell@example.com" `
        -Password "PowerShell@123"
    Write-Host "New user registered successfully!" -ForegroundColor Green
}
catch {
    Write-Host "Registration failed (user might already exist): $_" -ForegroundColor Yellow
}

# Example 3: Get all clients
Write-Host "`n=== Example 3: Get All Clients ===" -ForegroundColor Cyan
...
# Example 4: Get a specific client by ID
Write-Host "`n=== Example 4: Get Client by ID ===" -ForegroundColor Cyan
...
# Example 5: Add a new client
Write-Host "`n=== Example 5: Add New Client ===" -ForegroundColor Cyan
$newClient = @{
    ShortName = "PS"
...
# Example 6: Update a client
Write-Host "`n=== Example 6: Update Client ===" -ForegroundColor Cyan
...
# Example 7: Get all workers
Write-Host "`n=== Example 7: Get All Workers ===" -ForegroundColor Cyan
...
# Example 8: Add a new worker
Write-Host "`n=== Example 8: Add New Worker ===" -ForegroundColor Cyan
$newWorker = @{
...
# Example 9: Delete the test client (cleanup)
Write-Host "`n=== Example 9: Delete Test Client ===" -ForegroundColor Cyan
...
# Example 10: Delete the test worker (cleanup)
Write-Host "`n=== Example 10: Delete Test Worker ===" -ForegroundColor Cyan
